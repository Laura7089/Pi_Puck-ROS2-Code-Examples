    ...
    def __publish_laserscan_data(self):
        ...
        for i, key in enumerate(self.tof_sensors):
            dist_tofs[i] = interpolate_lookup_table(
                self.tof_sensors[key].getValue(),
                self.tof_sensors[key].getLookupTable())
        ...
        tof_dists = [
            OUT_OF_RANGE if dist > TOF_SHORT_MODE_MAX_RANGE else dist
            for dist in dist_tofs
        ]
        ...
        msg.ranges = [
            ...
            OUT_OF_RANGE,  # -105
            tof_dists[3] +
            SENSOR_DIST_FROM_CENTER,  # -90 tof ~changes when low
            OUT_OF_RANGE,  # -75
            OUT_OF_RANGE,  # -60
            tof_dists[4] +
            SENSOR_DIST_FROM_CENTER,  # -45 tof ~changes when low
            OUT_OF_RANGE,  # -30
            laser_dists[0],  # -15
